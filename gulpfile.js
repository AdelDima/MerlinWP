...
var buildFiles = [
  ...
  "!README.md",
  "!assets/scss/**",
  "!merlin-config-sample.php",
  "!merlin-filters-sample.php",
  "!CODE_OF_CONDUCT.md"
];
var buildDestination =
  "../themes/noor/framework/functions/admin/includes/merlin/";
...
gulp.task(
  "default",
  ["clear", "styles", "scripts", "browser_sync"],
  function() {
...
gulp.task("build-zip", ["build-variables"], function() {
  return gulp
    .src(buildDestination + "/**", { base: "dist" })
    .pipe(zip("merlin.zip"))
    .pipe(gulp.dest("./dist/"));
});
...
gulp.task("build-clean-after-zip", ["build-zip"], function() {
  return gulp
    .src([buildDestination, "!/dist/" + slug + "-wp.zip"], { read: false })
    .pipe(cleaner());
});

gulp.task(
  "build-zip-and-clean",
  ["build-zip", "build-clean-after-zip"],
  function() {}
);
...
gulp.task("build-noor", function(callback) {
  runSequence(
    "clear",
    "build-clean",
    ["styles", "scripts"],
